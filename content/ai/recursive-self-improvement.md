...
**Data.** Capability has been fuelled by enormous quantities of high-quality human text, and the best of that corpus is largely consumed. The obvious workaround - have the model generate its own training data - runs into model collapse: train a system mostly on its own output and quality degrades rather than compounds. A self-improving AI cannot simply imagine the new information it needs. Genuinely new knowledge about the world has to come from the world, through experiment and observation, and that is slow.
